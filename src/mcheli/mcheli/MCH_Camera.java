...
public class MCH_Camera
{
  ...
  public int getModeNum(int uid)
  {
    ...
    return this.canUseShader[uid] != false ? 3 : 2;
  }
  
  public int getMode(int uid) { return isValidUid(uid) ? this.mode[uid] : 0; }
  ...
  {
    if ((!isValidUid(uid)) || (viewer == null)) return;
    if ((W_Lib.isEntityLivingBase(viewer)) && (!viewer.isDead))
    {

      if ((getMode(uid) == 0) && (this.lastMode[uid] != 0))
      {
        PotionEffect pe = W_Entity.getActivePotionEffect(viewer, Potion.nightVision);
  ...
  public float getCameraZoom() { return this.zoom; }
}
