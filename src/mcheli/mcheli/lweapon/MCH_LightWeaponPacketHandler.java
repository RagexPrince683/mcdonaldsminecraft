...
{
  public MCH_LightWeaponPacketHandler() {}
  ...
  public static void onPacket_PlayerControl(EntityPlayer player, ByteArrayDataInput data)
  {
    ...
    if (is == null) return;
    if (!(is.getItem() instanceof MCH_ItemLightWeaponBase)) { return;
    }
    ...
    if (pc.camMode > 0) { MCH_Lib.DbgLog(false, "MCH_LightWeaponPacketHandler NV=%s", new Object[] { pc.camMode == 2 ? "ON" : "OFF" });
    }
    ...
    else if (pc.cmpReload > 0)
    {
      ...
      {
        if (W_EntityPlayer.hasItem(player, lweapon.bullet))
        {
          if (!player.capabilities.isCreativeMode)
          {
