  ...
  extends W_Item
{
  ...
  public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
    ...
    if ((!player.worldObj.isRemote) && (count < getMaxItemUseDuration(stack)) && (count % 20 == 0))
    {
      MCH_EntityAircraft ac = getMouseOverAircraft(player);
      ...
      if ((ac != null) && (ac.getHP() > 0) && (ac.repair(10)))
      {
        ...
        W_WorldFunc.MOD_playSoundEffect(player.worldObj, (int)ac.posX, (int)ac.posY, (int)ac.posZ, "wrench", 1.0F, 0.9F + rand.nextFloat() * 0.2F);
      }
    }
  }
  ...
  {
    MovingObjectPosition m = getMouseOver(player, 1.0F);
  ...
  private MovingObjectPosition getMouseOver(EntityLivingBase user, float tick)
  {
    ...
    double d1 = d0;
    Vec3 vec3 = Vec3.createVectorHelper(user.posX, user.posY + user.getEyeHeight(), user.posZ);
    ...
    Vec3 vec33 = null;
    float f1 = 1.0F;
    List list = user.worldObj.getEntitiesWithinAABBExcludingEntity(user, user.boundingBox.addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f1, f1, f1));
  ...
  public Multimap getItemAttributeModifiers()
  {
    ...
    multimap.put(SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), new AttributeModifier(field_111210_e, "Weapon modifier", this.damageVsEntity, 0));
