  ...
  extends W_Item
{
  ...
  public static boolean canUse(EntityPlayer player)
  {
    ...
    if (!(player.getCurrentEquippedItem().getItem() instanceof MCH_ItemRangeFinder)) { return false;
    }
    if ((player.ridingEntity instanceof MCH_EntityAircraft)) return false;
  ...
  public void spotEntity(EntityPlayer player, ItemStack itemStack)
  {
    ...
    {
      if ((rangeFinderUseCooldown == 0) && (player.getItemInUseDuration() > 8))
      {
        if (mode == 2)
        {
