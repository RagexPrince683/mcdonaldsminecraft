...
public class MCH_ClientLightWeaponTickHandler extends MCH_ClientTickHandlerBase
{
  ...
  private static java.nio.IntBuffer matViewport = BufferUtils.createIntBuffer(16);
  ...
  public static Vec3 getMartEntityPos() {
    ...
    {
      return Vec3.createVectorHelper(screenPos.get(0), screenPos.get(1), screenPos.get(2));
    }
    return null;
  }
  ...
  public static Vec3 getMartEntityBBPos() {
    if ((gs.getLockingEntity() == markEntity) && (markEntity != null))
    {
      return Vec3.createVectorHelper(screenPosBB.get(0), screenPosBB.get(1), screenPosBB.get(2));
    }
    return null;
  }
  ...
  protected void onTick(boolean inGUI)
  {
    ...
    {
      initWeaponParam(player);
      this.prevThePlayer = player;
    }
    
    ItemStack is = player != null ? player.getHeldItem() : null;
    
    if ((player == null) || ((player.ridingEntity instanceof mcheli.gltd.MCH_EntityGLTD)) || ((player.ridingEntity instanceof MCH_EntityAircraft)))
    {
    ...
    if ((is != null) && ((is.getItem() instanceof MCH_ItemLightWeaponBase)))
    {
      ...
      if ((this.prevItemStack == null) || ((!this.prevItemStack.isItemEqual(is)) && (!this.prevItemStack.getUnlocalizedName().equals(is.getUnlocalizedName()))))
      {
        ...
        weapon = mcheli.weapon.MCH_WeaponCreator.createWeapon(player.worldObj, MCH_ItemLightWeaponBase.getName(is), Vec3.createVectorHelper(0.0D, 0.0D, 0.0D), 0.0F, 0.0F, null, false);
      ...
      {
        lockonSoundCount = 0;
        if ((mcheli.wrapper.W_EntityPlayer.hasItem(player, lweapon.bullet)) && (player.getItemInUseCount() <= 0))
        {
          if (reloadCount == 10)
          {
      ...
      if (!inGUI) { playerControl(player, is, (MCH_ItemLightWeaponBase)is.getItem());
      }
    ...
    if ((this.KeyAttack.isKeyPress()) || (autoShot))
    {
      ...
      if (is.getItemDamage() < is.getMaxDamage())
      {
        ...
        if (gs.isLockComplete())
        {
          ...
          {
            pc.useWeapon = true;
            pc.useWeaponOption1 = mcheli.wrapper.W_Entity.getEntityId(gs.lastLockEntity);
      ...
      {
        if ((!result) && (player.getItemInUseDuration() > 5))
        {
          playSoundNG();
        }
      }
