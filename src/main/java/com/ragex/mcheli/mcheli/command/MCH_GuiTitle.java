  ...
  extends MCH_Gui
{
  ...
  public void setupTitle(IChatComponent chatComponent, int showTime, int pos)
  {
    ...
    int k = MathHelper.floor_float(func_146233_a() / this.mc.gameSettings.chatScale);
    int l = 0;
    ...
    for (int i1 = 0; i1 < arraylist1.size(); i1++)
    {
      ...
      String[] splitLine = (ichatcomponent1.getUnformattedTextForChat() + "").split("\n");
      int lineCnt = 0;
      for (String sLine : splitLine)
      {
        String s = func_146235_b(ichatcomponent1.getChatStyle().getFormattingCode() + sLine);
    ...
    IChatComponent ichatcomponent2; for (; iterator.hasNext(); 
