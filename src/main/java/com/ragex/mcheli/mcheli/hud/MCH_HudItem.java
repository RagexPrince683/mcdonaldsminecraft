  ...
  extends Gui
{
  ...
  protected static Map<String, Double> varMap = null;
  ...
  public void drawTexture(String name, double left, double top, double width, double height, double uLeft, double vTop, double uWidth, double vHeight, float rot, int textureWidth, int textureHeight)
  {
    ...
    tessellator.addVertexWithUV(-width / 2.0D, height / 2.0D, this.zLevel, uLeft * fx, (vTop + vHeight) * fy);
    tessellator.addVertexWithUV(width / 2.0D, height / 2.0D, this.zLevel, (uLeft + uWidth) * fx, (vTop + vHeight) * fy);
    tessellator.addVertexWithUV(width / 2.0D, -height / 2.0D, this.zLevel, (uLeft + uWidth) * fx, vTop * fy);
    tessellator.addVertexWithUV(-width / 2.0D, -height / 2.0D, this.zLevel, uLeft * fx, vTop * fy);
    tessellator.draw();
  ...
  public static void updateVarMap(MCH_EntityAircraft ac, MCH_WeaponSet ws)
  {
    ...
    updateVarMapItem("time", player.worldObj.getWorldTime() % 24000L);
  ...
  public static void updateVarMapItem(String key, double value) { varMap.put(key, Double.valueOf(value)); }
  
  public static void drawVarMap() { int i;
    int x;
    int y;
    if (MCH_Config.TestMode.prmBool)
    {
      i = 0;
      x = (int)(-300.0D + centerX);
  ...
  private static void updateVarMap_Weapon(MCH_WeaponSet ws) {
    ...
    if (ws != null)
    {
      ...
      if (wi.delay > wi.reloadTime)
      {
        ...
        if (rel_time < 0.0F) rel_time = -rel_time;
        if (rel_time > 1.0F) rel_time = 1.0F;
      }
      else
      {
  ...
  public static int isLowFuel(MCH_EntityAircraft ac)
  {
    ...
    {
      if ((ac.getMaxFuel() > 0) && (ac.getFuelP() < 0.1F) && (!ac.isInfinityFuel(player, false)))
      {
        is_low_fuel = 1;
      }
    }
  ...
  public static int getFreeLook(MCH_EntityAircraft ac, EntityPlayer player) {
    if ((ac.isPilot(player)) && (ac.canSwitchFreeLook()) && (ac.isFreeLookMode()))
    {
      return 1;
    }
    return 0;
  ...
  public static int getAutoPilot(MCH_EntityAircraft ac, EntityPlayer player) {
    if ((ac instanceof MCP_EntityPlane))
    {
      if ((ac.isPilot(player)) && (ac.getIsGunnerMode(player)))
      {
        return 1;
      }
    }
    ...
    if (altitudeUpdateCount <= 0)
    {
      ...
      for (int i = 0; i < 256; i++)
      {
        ...
        if (id != 0)
        {
          ...
          if (ac.posY <= 256.0D)
            break;
          Altitude = (int)(Altitude + (ac.posY - 256.0D)); break;
        }
      }
  ...
  public static void updateWeapon(MCH_EntityAircraft ac, MCH_WeaponSet ws)
  {
    ...
    if (wi.delay > wi.reloadTime)
    {
      ...
      ReloadPer = ws.countWait / (wi.delay > 0 ? wi.delay : 1);
      if (ReloadPer < 0.0F) ReloadPer = -ReloadPer;
      if (ReloadPer > 1.0F) ReloadPer = 1.0F;
    }
    else
    {
      ReloadSec = ws.countReloadWait;
      ReloadPer = ws.countReloadWait / (wi.reloadTime > 0 ? wi.reloadTime : 1);
    }
    ReloadSec /= 20.0F;
