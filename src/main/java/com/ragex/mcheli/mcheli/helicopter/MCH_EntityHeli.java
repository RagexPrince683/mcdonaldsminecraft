...
public class MCH_EntityHeli extends MCH_EntityAircraft
{
  ...
  {
    MCH_Lib.DbgLog(this.worldObj, "MCH_EntityHeli.changeType " + type + " : " + toString(), new Object[0]);
    ...
    {
      MCH_Lib.Log(this, "##### MCH_EntityHeli changeHeliType() Heli info null %d, %s, %s", new Object[] { Integer.valueOf(W_Entity.getEntityId(this)), type, getEntityName() });
  ...
  public Item getItem()
  {
    return getHeliInfo() != null ? getHeliInfo().item : null;
  }
  ...
  {
    super.entityInit();
    this.dataWatcher.addObject(30, Byte.valueOf((byte)2));
  }
  ...
  protected void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
  {
    ...
    if (this.heliInfo == null)
    {
      ...
      if (this.heliInfo == null)
      {
        MCH_Lib.Log(this, "##### MCH_EntityHeli readEntityFromNBT() Heli info null %d, %s", new Object[] { Integer.valueOf(W_Entity.getEntityId(this)), getEntityName() });
  ...
  public boolean isFoldBlades() {
    if ((this.heliInfo == null) || (this.rotors.length <= 0)) return false;
  ...
  {
    if (this.heliInfo == null) return false;
    if (this.rotors.length <= 0) return true;
    if (getFoldBladeStat() == 2)
    {
      for (MCH_Rotor r : this.rotors)
      {
        if (r.isFoldingOrUnfolding()) return false;
      }
      return true;
    }
    return false;
  ...
  protected void foldBlades() {
    if ((this.heliInfo == null) || (this.rotors.length <= 0)) return;
  ...
  public float getControlRotYaw(float mouseX, float mouseY, float tick) { return mouseX; }
  public float getControlRotPitch(float mouseX, float mouseY, float tick) { return mouseY; }
  public float getControlRotRoll(float mouseX, float mouseY, float tick) { return mouseX; }
  ...
  public void onUpdateAngles(float partialTicks)
  {
    ...
    if (MCH_ServerSettings.enableRotationLimit)
    {
      ...
      {
        setRotPitch(getRotPitch() - Math.abs((getRotPitch() - MCH_ServerSettings.pitchLimitMax) * 0.1F * partialTicks));
      }
      if (getRotPitch() < MCH_ServerSettings.pitchLimitMin)
      {
        setRotPitch(getRotPitch() + Math.abs((getRotPitch() - MCH_ServerSettings.pitchLimitMin) * 0.2F * partialTicks));
      }
      if (getRotRoll() > MCH_ServerSettings.rollLimit)
      {
        setRotRoll(getRotRoll() - Math.abs((getRotRoll() - MCH_ServerSettings.rollLimit) * 0.03F * partialTicks));
      }
      if (getRotRoll() < -MCH_ServerSettings.rollLimit)
      {
        setRotRoll(getRotRoll() + Math.abs((getRotRoll() + MCH_ServerSettings.rollLimit) * 0.03F * partialTicks));
  ...
  protected void onUpdate_Control()
  {
    ...
    {
      if (getCurrentThrottle() > 0.0D) addCurrentThrottle(-0.00125D); else {
        setCurrentThrottle(0.0D);
      }
      if ((this.heliInfo.isEnableFoldBlade) && (this.rotors.length > 0) && (getFoldBladeStat() == 0) && (this.onGround) && (!isDestroyed()))
      {
    ...
    if (this.worldObj.isRemote)
    {
      ...
      if (!W_Lib.isClientPlayer(getRiddenByEntity()))
      {
        ...
        if (getCurrentThrottle() >= ct - 0.02D) { addCurrentThrottle(-0.01D);
        } else if (getCurrentThrottle() < ct) { addCurrentThrottle(0.01D);
        }
  ...
  protected void onUpdate_ControlNotHovering()
  {
    ...
    if (this.throttleUp)
    {
      if (getCurrentThrottle() < 1.0D) addCurrentThrottle(0.02D * throttleUpDown); else {
        setCurrentThrottle(1.0D);
      }
    } else if (this.throttleDown)
    {
      if (getCurrentThrottle() > 0.0D) addCurrentThrottle(-0.014285714285714285D * throttleUpDown); else {
        setCurrentThrottle(0.0D);
      }
    } else if ((!this.worldObj.isRemote) || (W_Lib.isClientPlayer(getRiddenByEntity())))
    {
  ...
  {
    if (getCurrentThrottle() < 1.0D) addCurrentThrottle(0.03333333333333333D); else {
      setCurrentThrottle(1.0D);
    }
    if (!this.worldObj.isRemote)
    {
  ...
  protected void onUpdate_Particle2()
  {
    ...
    if (getHeliInfo() == null) return;
    int rotorNum = getHeliInfo().rotorList.size();
    if (rotorNum <= 0) { return;
    }
    ...
    {
      Vec3 rotor_pos = ((MCH_HeliInfo.Rotor)getHeliInfo().rotorList.get(ri)).pos;
      
      float yaw = getRotYaw();
      float pitch = getRotPitch();
      
      Vec3 pos = MCH_Lib.RotVec3(rotor_pos, -yaw, -pitch, -getRotRoll());
      ...
      for (double i = 0.0D; i < num; i += 1.0D)
      {
        ...
        if (p < this.rand.nextFloat() / 2.0F)
        {
          ...
          MCH_ParticleParam prm = new MCH_ParticleParam(this.worldObj, "smoke", prev.xCoord + (x - prev.xCoord) * (i / num), prev.yCoord + (y - prev.yCoord) * (i / num), prev.zCoord + (z - prev.zCoord) * (i / num));
  ...
  private void onUpdate_Server()
  {
    ...
    if (!isHovering())
    {
      ...
      if (dp == 0.0D)
      {
        ...
        this.motionX += 0.1D * MathHelper.sin(yaw) * this.currentSpeed * -(pitch * pitch * pitch / 20000.0F) * getCurrentThrottle();
        this.motionZ += 0.1D * MathHelper.cos(yaw) * this.currentSpeed * (pitch * pitch * pitch / 20000.0F) * getCurrentThrottle();
        double y = 0.0D;
    ...
    {
      if (this.rand.nextInt(50) == 0) this.motionX += (this.rand.nextDouble() - 0.5D) / 30.0D;
      if (this.rand.nextInt(50) == 0) this.motionY += (this.rand.nextDouble() - 0.5D) / 50.0D;
      if (this.rand.nextInt(50) == 0) { this.motionZ += (this.rand.nextDouble() - 0.5D) / 30.0D;
      }
    }
