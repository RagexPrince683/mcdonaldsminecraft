...
public class MCH_ItemChain extends W_Item
{
  ...
  public static void interactEntity(ItemStack item, Entity entity, EntityPlayer player, World world)
  {
    if ((!world.isRemote) && (entity != null) && (!entity.isDead))
    {
      ...
      if ((entity instanceof MCH_EntityChain)) return;
      if ((entity instanceof MCH_EntityHitBox)) return;
      if ((entity instanceof MCH_EntitySeat)) return;
      if ((entity instanceof mcheli.uav.MCH_EntityUavStation)) return;
      if ((entity instanceof MCH_EntityParachute)) return;
      if (mcheli.wrapper.W_Lib.isEntityLivingBase(entity)) { return;
      }
  ...
  public static MCH_EntityChain getTowedEntityChain(Entity entity)
  {
    ...
    for (int i = 0; i < list.size(); i++)
    {
      ...
      if (chain.isTowingEntity())
      {
        ...
        if (W_Entity.isEqual(chain.towEntity, entity)) return chain;
        if (W_Entity.isEqual(chain.towedEntity, entity)) return chain;
      }
    }
    return null;
  }
